...
interface WhatsAppFloatProps {
  /** Already-translated product title; when present the message is contextual. */
  productTitle?: string | null;
}

const WhatsAppFloat = ({ productTitle }: WhatsAppFloatProps) => {
  ...
  const buildMessage = () => {
    ...
    if (title) {
      ...
      const base = t("whatsapp.productMessage", { title }) as string;
      return url ? `${base}\n\n${url}` : base;
    }
    return t("whatsapp.genericMessage") as string;
  };
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      aria-label={t("whatsapp.ariaLabel") as string}
      className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-40 flex items-center justify-center h-12 w-12 md:h-14 md:w-14 rounded-full bg-[#25D366] text-white shadow-lg transition-all duration-300 hover:bg-[#1ebe5d] hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#25D366] focus-visible:ring-offset-2"
    >
