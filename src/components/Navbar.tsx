      ...
      <div className="container mx-auto px-6">
        <div className="flex justify-between items-center py-4 mx-0">
          ...
          <Link to="/" className="flex items-center space-x-2 group">
            <div className={`p-2 rounded-lg transition-colors ${isScrolled ? 'bg-white group-hover:bg-gray-100' : 'bg-blue-600 group-hover:bg-blue-700'}`}>
              <Truck className={`h-6 w-6 ${isScrolled ? 'text-orange-500' : 'text-white'}`} />
            </div>
          ...
          <div className="hidden lg:flex items-center space-x-1 ml-auto">
            {navItems.map(item => <Link key={item.path} to={item.path} className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 ${isActive(item.path) ? isScrolled ? "bg-orange-600 text-white" : "bg-white/20 text-white" : isScrolled ? "text-white hover:bg-orange-600 hover:text-white" : "text-white hover:bg-white/10 hover:text-white"}`}>
                {item.name}
              </Link>)}
          </div>
          ...
          <div className="lg:hidden">
            <button onClick={() => setIsOpen(!isOpen)} className={`p-2 rounded-lg transition-colors ${isScrolled ? 'hover:bg-orange-600 text-white' : 'hover:bg-white/10 text-white'}`}>
